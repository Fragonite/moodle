...
class provider_test extends provider_testcase {
    ...
    public function test_add_contexts_for_userid(): void {
        ...
        // exemption 2 courses for user1 and 1 course for user2, all at the site context.
        $ufservice1 = \core_exemptions\service_factory::get_service_for_user_context($user1context);
        $ufservice2 = \core_exemptions\service_factory::get_service_for_user_context($user2context);
        $systemcontext = \context_system::instance();
        $ufservice1->create_exemption('core_course', 'courses', $course1context->instanceid, $systemcontext);
        $ufservice1->create_exemption('core_course', 'courses', $course2context->instanceid, $systemcontext);
        $ufservice2->create_exemption('core_course', 'courses', $course2context->instanceid, $systemcontext);
        $this->assertCount(2, $ufservice1->find_exemptions_by_type('core_course', 'courses'));
        $this->assertCount(1, $ufservice2->find_exemptions_by_type('core_course', 'courses'));

        // Now, just for variety, let's assume you can exemption a course at user context, and do so for user1.
        $ufservice1->create_exemption('core_course', 'courses', $course1context->instanceid, $user1context);
    ...
    public function test_delete_exemptions_for_user(): void {
        ...
        // exemption 2 courses for user1 and 1 course for user2, all at the user context.
        $ufservice1 = \core_exemptions\service_factory::get_service_for_user_context($user1context);
        $ufservice2 = \core_exemptions\service_factory::get_service_for_user_context($user2context);
        $ufservice1->create_exemption('core_course', 'courses', $course1context->instanceid, $user1context);
        $ufservice1->create_exemption('core_course', 'courses', $course2context->instanceid, $user1context);
        $ufservice2->create_exemption('core_course', 'courses', $course2context->instanceid, $user2context);
        $this->assertCount(2, $ufservice1->find_exemptions_by_type('core_course', 'courses'));
        $this->assertCount(1, $ufservice2->find_exemptions_by_type('core_course', 'courses'));

        // Now, delete the exemptions for user1 only.
        $approvedcontextlist = new \core_privacy\local\request\approved_contextlist($user1, 'core_course', [$user1context->id]);
        provider::delete_exemptions_for_user($approvedcontextlist, 'core_course', 'courses');

        // Verify that we have no exemption courses for user1 but that the records are in tact for user2.
        $this->assertCount(0, $ufservice1->find_exemptions_by_type('core_course', 'courses'));
        $this->assertCount(1, $ufservice2->find_exemptions_by_type('core_course', 'courses'));
    }
    ...
    public function test_delete_exemptions_for_all_users(): void {
        ...
        // exemption 2 course modules for user1 and 1 course module for user2 all in course 1 context.
        $ufservice1 = \core_exemptions\service_factory::get_service_for_user_context($user1context);
        $ufservice2 = \core_exemptions\service_factory::get_service_for_user_context($user2context);
        $ufservice1->create_exemption('core_course', 'modules', 1, $course1context);
        $ufservice1->create_exemption('core_course', 'modules', 2, $course1context);
        $ufservice2->create_exemption('core_course', 'modules', 3, $course1context);

        // Now, exemption a different course module for user2 in course 2.
        $ufservice2->create_exemption('core_course', 'modules', 5, $course2context);

        $this->assertCount(2, $ufservice1->find_exemptions_by_type('core_course', 'modules'));
        $this->assertCount(2, $ufservice2->find_exemptions_by_type('core_course', 'modules'));

        // Now, delete all course module exemptions in the 'course1' context only.
        ...
        $this->assertCount(0, $ufservice1->find_exemptions_by_type('core_course', 'modules'));
        $this->assertCount(1, $ufservice2->find_exemptions_by_type('core_course', 'modules'));
    }
    ...
    public function test_delete_exemptions_for_userlist(): void {
        ...
        // exemption 2 courses for user1 and 1 course for user2.
        $systemcontext = \context_system::instance();
        $ufservice1 = \core_exemptions\service_factory::get_service_for_user_context($user1context);
        $ufservice2 = \core_exemptions\service_factory::get_service_for_user_context($user2context);
        $ufservice1->create_exemption('core_course', 'courses', $course1context->instanceid, $systemcontext);
        ...
        // Ask the exemptions privacy api to export userids for exemptions of the type we just created, in the system context.
        ...
        // Now, delete the exemptions for user1 only in the system context.
        $approveduserlist = new \core_privacy\local\request\approved_userlist($systemcontext, 'core_course',
                [$user1->id]);
        provider::delete_exemptions_for_userlist($approveduserlist, 'courses');
        ...
        // Now, delete the exemptions for user2 only in the user1 context.
        // Make sure exemptions are only being deleted in the right context.
    ...
    public function test_get_exemptions_info_for_user(): void {
        ...
        $ufservice1 = \core_exemptions\service_factory::get_service_for_user_context($user1context);
        $ufservice2 = \core_exemptions\service_factory::get_service_for_user_context($user2context);
        $courseexemption1 = $ufservice1->create_exemption('core_course', 'courses',
                $course1context->instanceid, $course1context);
        $this->waitForSecond();
        $courseexemption2 = $ufservice1->create_exemption('core_course', 'courses',
                $course2context->instanceid, $course2context);
        $this->waitForSecond();
        $courseexemption3 = $ufservice2->create_exemption('core_course', 'courses',
                $course2context->instanceid, $course2context);
        $this->assertCount(2, $ufservice1->find_exemptions_by_type('core_course', 'courses'));
        $this->assertCount(1, $ufservice2->find_exemptions_by_type('core_course', 'courses'));

        // Get the exemptions info for user1 in the course1 context.
        $exemptioninfo1 = (object) provider::get_exemptions_info_for_user($user1->id, $course1context,
                'core_course', 'courses', $course1context->instanceid);
        // Ensure the correct data has been returned.
        $this->assertEquals(transform::yesno(true), $exemptioninfo1->starred);
        $this->assertEquals('', $exemptioninfo1->ordering);
        $this->assertEquals(transform::datetime($courseexemption1->timecreated), $exemptioninfo1->timecreated);
        $this->assertEquals(transform::datetime($courseexemption1->timemodified), $exemptioninfo1->timemodified);

        // Get the exemptions info for user1 in the course2 context.
        $exemptioninfo2 = (object) provider::get_exemptions_info_for_user($user1->id, $course2context,
                'core_course', 'courses', $course2context->instanceid);
        // Ensure the correct data has been returned.
        $this->assertEquals(transform::yesno(true), $exemptioninfo2->starred);
        $this->assertEquals('', $exemptioninfo2->ordering);
        $this->assertEquals(transform::datetime($courseexemption2->timecreated), $exemptioninfo2->timecreated);
        $this->assertEquals(transform::datetime($courseexemption2->timemodified), $exemptioninfo2->timemodified);

        // Get the exemptions info for user2 in the course2 context.
        $exemptioninfo3 = (object) provider::get_exemptions_info_for_user($user2->id, $course2context,
                'core_course', 'courses', $course2context->instanceid);
        // Ensure the correct data has been returned.
        $this->assertEquals(transform::yesno(true), $exemptioninfo3->starred);
        $this->assertEquals('', $exemptioninfo3->ordering);
        $this->assertEquals(transform::datetime($courseexemption3->timecreated), $exemptioninfo3->timecreated);
        $this->assertEquals(transform::datetime($courseexemption3->timemodified), $exemptioninfo3->timemodified);

        // Get the exemptions info for user2 in the course1 context (user2 has not exempt course1).
        $exemptioninfo4 = provider::get_exemptions_info_for_user($user2->id, $course1context,
                'core_course', 'courses', $course1context->instanceid);
        // Ensure that data has not been returned.
        $this->assertEmpty($exemptioninfo4);
    }
}
